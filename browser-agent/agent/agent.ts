...
export async function runAgentLoop(tabId: number, instruction: AgentInstruction, getRunning: () => boolean): Promise<AgentRuntimeState> {
  ...
  while (getRunning()) {
    ...
    try {
      ...
      for (const job of jobs) {
        if (!getRunning() || state.appliedCount >= instruction.maxApplications) break;
